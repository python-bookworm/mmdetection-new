...
model = dict(
    ...
    bbox_head=[
        dict(
            ...
            num_classes=5,# 分类器的类别数量+1，+1是因为多了一个背景的类别
            target_means=[0., 0., 0., 0.],# 均值
...
data = dict(
    ...
    train=dict(
        ...
        ann_file=data_root + 'annotations/train/train.json',# 数据集annotation路径
        img_prefix=data_root + 'train2017/',# 数据集的图片路径
        pipeline=train_pipeline),
    val=dict(
        type=dataset_type,
        ann_file=data_root + 'annotations/val/val.json',
    ...
    test=dict(
        ...
        pipeline=test_pipeline))
# optimizer
optimizer = dict(type='SGD', lr=0.0125, momentum=0.9, weight_decay=0.0001)  # 优化参数，lr为学习率，momentum为动量因子，weight_decay为权重衰减因子
